...
#复制CommonsSDK.framework到打包目录
FRAMEWORKPATH="$PROJECT_DIR"/../CommonsSDK/CommonsSDK.embeddedframework/CommonsSDK.framework
cp -rf "$FRAMEWORKPATH" "$CONFIGURATION_BUILD_DIR"

#Run the ADT command to generate the ANE
pushd "$CONFIGURATION_BUILD_DIR"
"$AIR_SDK_PATH"/bin/adt -package -target ane "$TARGET_NAME" extension.xml -swc "$EXTENSION_SWC_FILE_NAME" -platform default library.swf -platform iPhone-ARM -platformoptions platformoptions.xml "$NATIVE_EXTENSION_STATIC_LIB_NAME" CommonsSDK.framework library.swf
popd
